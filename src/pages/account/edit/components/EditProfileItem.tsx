...
const EditProfileItem = ({ profile, onResetProfile, title, oldValue }: EditProfileItemProps) => {
    ...
    const updateProperty = async () => {
        ...
        try {
            const resp: any = await editUserProfile({ ...profile, [title]: value });
            onResetProfile && onResetProfile({ ...profile, [title]: value });
            ...
            updateUserLocally({ ...profile, [title]: value });
    ...
    return (
        <form>
            ...
            <input type="text" name={title} placeholder={oldValue || "Enter Here"} value={value} onChange={handleChange}/>
            <div className={`${styles.error_label} ${errorMessage && styles.active}`}>
              {errorMessage || "Invalid Property"}
            </div>
            <AsyncButton onClick={updateProperty} className={styles.submit_btn} isLoading={isLoading}>Save</AsyncButton>
            <Button onClick={goToProfilePage} className={styles.cancel_btn}>Cancel</Button>
        </form>
    )
}
